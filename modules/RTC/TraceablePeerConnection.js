...
//  import SdpConsistency from '../xmpp/SdpConsistency';    //  hasevr in EXT_MULTI_VIDEO ssrc should not be modified.
...
export default function TraceablePeerConnection(
    ...
    const console_log = console.log;    // hasevr suppress log message
    ...
    //  hasevr  this.sdpConsistency = new SdpConsistency(this.toString());
...
TraceablePeerConnection.prototype._peerVideoTypeChanged = function(
    ...
    // Check if endpointId has a value to avoid action on random track
    if (!endpointId) {
        logger.error('TPC _peerVideoTypeChanged: No endpointID');
...
TraceablePeerConnection.prototype.getRemoteTracks = function(
    ...
    for (const endpoint of endpoints) {
		...
		//	EXT_MULTI_VIDEO ------------------------------------
/*
        for (const trackMediaType of remoteTrackMap.keys()) {
            // per media type filtering
            if (!mediaType || mediaType === trackMediaType) {
                const mediaTrack = remoteTrackMap.get(trackMediaType);
...
TraceablePeerConnection.prototype._remoteTrackAdded = function(stream, track, transceiver = null) {
    ...
    if (peerMediaInfo.videoTypes){
        this.remoteVideoTypeMaps.set(ownerEndpointId, peerMediaInfo.videoTypes)
    }
    this._createRemoteTrack(
        ownerEndpointId, stream, track, mediaType, ssrcs, muted);
};
...
TraceablePeerConnection.prototype._createRemoteTrack = function(
	...
	//	EXT_MULTI_VIDEO ------------------------------------
/*    const existingTrack = remoteTrackMap.get(mediaType);
...
TraceablePeerConnection.prototype.addTrack = function(track, isInitiator = false) {
    ...
    if (browser.usesPlanB() && browser.doesVideoMuteByStreamRemove()
        ...
        //  hasevr  this.sdpConsistency.setPrimarySsrc(ssrcInfo.ssrcs[0]);
        const simGroup
...
TraceablePeerConnection.prototype.addTrackUnmute = function(track) {
    ...
    //  hasevr  sendVideoTypes when finished to add track and renegotiate ssrc.
    //track.storedMSID = track.getMSID(); //  update track id
    track.once(TRACK_MUTE_CHANGED, () => { this.sendVideoTypes(); })
...
TraceablePeerConnection.prototype.generateRecvonlySsrc = function() {
    ...
    //  hasevr  this.sdpConsistency.setPrimarySsrc(newSSRC);
};
...
TraceablePeerConnection.prototype.clearRecvonlySsrc = function() {
    ...
    //  hasevr  this.sdpConsistency.clearVideoSsrcCache();
};
        ...
        try {
            ...
            */
            ...
            if (this.isSimulcastOn() && browser.usesSdpMungingForSimulcast()
                ...
                const console_log = console.log; // hasevr suppress console.log
                console.log = console.debug;     //
                // eslint-disable-next-line no-param-reassign
                resultSdp = this.simulcast.mungeLocalDescription(resultSdp);
                console.log = console_log;       // hasevr restore console.log
...
TraceablePeerConnection.prototype.getPrimarySsrc = function(rtcId) {
    const ssrcInfo = this.localSSRCs.get(rtcId);
